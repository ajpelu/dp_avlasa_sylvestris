...
proto_parent_event <- 
  ...
  relocate(
    ...
    samplingProtocol, 
    year, month, day, 
    continent, country, countryCode, province, municipality, locality = location,
...
soil_event <- proto_parent_event |> 
  ...
  mutate(protoID = paste0(tree, "S"),
         ...
         habitat = "Soil of each target tree (Pinus sylvestris)") |> 
...
micro_event <- proto_parent_event |> 
  inner_join((bac |> dplyr::select(sample, tree) |> unique())) |> 
  dplyr::select(-parentEventID, -eventType, -samplingProtocol, -habitat) |> 
  rename(parentEventID = eventID) |> 
  ...
  mutate(
    ...
    samplingProtocol = case_when(
      target == "E" ~ "DNA extraction from the root endosphere of Pinus sylvestris trees during the spring, and sequencing of the 16S rRNA gene",
      target == "R" ~ "DNA extraction from the rhizosphere of Pinus sylvestris trees during the spring, and sequencing of the 16S rRNA gene"
    ),
    habitat = case_when(
...
df_idevents <- events |> 
  ...
  dplyr::select(eventID, habitat, license, institutionCode, language, datasetName, 
                continent, country, countryCode, province, municipality, locality,
...
occ_bac <- bac |> 
 filter(n > 0) |> 
  ...
  relocate(
    ...
    eventID, identifyBy, identifiedByID, taxonID, nameAccordingToID,
